...
const filterPrice =(price)=>{
  ...
  return (
    <div className="max-w-[1640px] m-auto px-4 py-12">
      ...
      <div className="flex flex-col lg:flex-row justify-between">
        ...
        <div>
          ...
          <div className="flex justify-between flex-wrap">
            <button onClick={()=> filterType(data)} className="m-1 border-orange-600 text-orange-600 hover:bg-orange-600 hover:text-white">
              All
            </button>
      ...
      {/* Display foods */}
      <div className="grid grid-cols-2 lg:first-letter:grid-cols-4 gap-6 pt-4">
      {foods.map((item, index)=>(
        <div key={index} className='border shadow-lg rounded-lg hover:scale-105 duration-300'>
          <img className="w-full h-[200px] object-cover rounded-t-large" src={item.image} alt={item.name}/>
